...
class DDMTD(Component):
    ...
    def elaborate(self, platform):
        ...
        #dbg = platform.request('dbg')
        #m.d.comb += dbg.dbg0.o.eq(mult_a)
        #m.d.comb += dbg.dbg1.o.eq(mult_b)
        ...
        m.d.comb += self.raw_b.eq(mult_b)

        cnt = Signal(16)#self.word_size)
        ...
        with m.If(deglitch_b.begin_glitches):
            m.d.sync += t.eq(cnt)
        with m.If(deglitch_b.end_glitches):
            #m.d.sync += self.phase.eq((t + deglitch_b.median)[:16])
            m.d.sync += self.phase.eq((deglitch_b.nb_transitions)[:16])
            #m.d.sync += self.phase.eq((cnt - t - 1000)[:16])
            m.d.sync += self.new_phase.eq(True)
        with m.If(self.new_phase):
            m.d.sync += self.new_phase.eq(False)
